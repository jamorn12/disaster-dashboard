...
with col_map:
    ...
    # 🛣️ 🌟 เลเยอร์เส้นทางสำรวจ (ดึงจากไฟล์ GPX โดยตรง)
    layer_route = folium.FeatureGroup(name="🛣️ เส้นทางสำรวจ (GPX)", show=True)
    if gpx_points:
        # วาดเส้นทึบสีฟ้าตามพิกัดในไฟล์ GPX
        folium.PolyLine(gpx_points, color='#3498db', weight=6, opacity=0.8).add_to(layer_route)
        # เพิ่มลูกเล่น AntPath (เส้นประวิ่ง) เพื่อความสวยงาม
        plugins.AntPath(gpx_points, color='#ffffff', weight=2).add_to(layer_route)
        
        # เพิ่ม Marker ที่จุดเริ่มต้นและจุดสิ้นสุดของไฟล์ GPX
        folium.Marker(gpx_points[0], popup="<b>จุดเริ่มต้นเส้นทาง</b>", icon=folium.Icon(color='green', icon='play')).add_to(layer_route)
        folium.Marker(gpx_points[-1], popup="<b>จุดสิ้นสุดเส้นทาง</b>", icon=folium.Icon(color='red', icon='stop')).add_to(layer_route)
