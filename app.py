...
# อัปเดตเป็น ID ไฟล์ใหม่ที่คุณแจ้ง (1storm_y2025_original.csv)
CSV_ID = '1ac8biU8i89KS0XEfjjg0Axa2vqPi8QXT' 
SHP_ZIP_ID = '1wFrYGQ6gUjhlDAuwfnGe1jIZ5cqU01aE' 
# ใช้ ID เดียวกับ CSV สำหรับ GPX หรือระบุ ID ของไฟล์ GPX จริงที่นี่
...
def load_all_data():
    ...
    # 🛠️ จุดแก้ไขสำคัญ: จัดการข้อมูลความเสียหายและตัดค่า 9999
    col_damage = 'บ้านเสียหาย \n(หลังคาเรือน)'
    df['บ้านเสียหายรวม'] = pd.to_numeric(df[col_damage], errors='coerce').fillna(0)
    # ตัดค่า 9999 ออก (เปลี่ยนเป็น 0 เพื่อไม่ให้ยอดรวมเพี้ยน)
