...
# 📍 ใส่ File ID ของไฟล์ GPX (mapstogpx...) จาก Google Drive ของคุณที่นี่
...
def load_all_data():
    ...
    if not os.path.exists("temp_shp"): os.makedirs("temp_shp")
    ...
    pv_col = next((c for c in ['PV_TN', 'PROV_NAMT', 'pro_tn'] if c in gdf.columns), gdf.columns[0])
    ap_col = next((c for c in ['AP_TN', 'AMP_NAMT', 'am_tn'] if c in gdf.columns), gdf.columns[1])
    ...
    # 🌟 GPX (เส้นสีฟ้า) - ดึงผ่าน Drive API
    gpx_pts = []
    try:
...
with col_map:
    ...
    for _, row in show_pts.iterrows():
        ...
        pop = f"<b>อ.{row['amp_clean']}</b><br>🏠 เสียหาย: {int(row['บ้านเสียหายรวม'])} หลัง<hr><a href='{g_url}' target='_blank' style='display:block; text-align:center; background:#4285F4; color:white; padding:5px; border-radius:5px; text-decoration:none;'>🚀 ไป Google Maps</a>"
    ...
    # 🌟 เลเยอร์ Waypoints (สถานที่สำคัญ 6 จุด)
    ...
    # 🛣️ เลเยอร์เส้นทางสำรวจ (เส้นสีฟ้า)
